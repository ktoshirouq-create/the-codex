document.addEventListener('DOMContentLoaded', () => {
    ...
    // BOUNCER QUICK CHECK (Live Date Engine)
    const updateBouncerDates = () => {
        ...
        document.getElementById('date-18').innerText = date18.toLocaleDateString('en-US', options);
        document.getElementById('date-20').innerText = date20.toLocaleDateString('en-US', options);
    };
    updateBouncerDates();
    ...
    let activeAbvSpec = null; let abvDilution = 20;
    ...
    document.getElementById('btn-abv-spec').addEventListener('click', () => {
        openSelectModal('SELECT SPEC FOR ABV', Object.keys(recipeVault).map(s => ({label: s, value: s})), (v, l) => {
            activeAbvSpec = v; document.getElementById('btn-abv-spec').innerText = l; document.getElementById('btn-abv-spec').style.color = "var(--text-main)"; document.getElementById('btn-abv-spec').classList.remove('text-muted');
            const list = document.getElementById('abv-ing-list'); list.innerHTML = '';
            recipeVault[v].forEach((ing, i) => {
                let defAbv = 0; if(ing.color === 'amber-glow') defAbv = 40; if(ing.color === 'neon-cyan') defAbv = 20; 
                list.innerHTML += `
            ...
            document.getElementById('abv-ing-container').classList.remove('hidden'); document.getElementById('abv-results').innerHTML = '';
        });
    });
    ...
    document.getElementById('calc-abv-btn').addEventListener('click', () => {
        ...
        document.querySelectorAll('.abv-param').forEach(inp => {
            const v = parseFloat(inp.getAttribute('data-vol')); totalVol += v; totalAlc += (v * ((parseFloat(inp.value)||0) / 100));
        });
    ...
    // --- THE LAB: WASHING ---
    let washType = 'milk'; let fatIntensity = 'standard';
    document.querySelectorAll('.wash-pill').forEach(p => p.addEventListener('click', (e) => {
        triggerHaptic('light'); document.querySelectorAll('.wash-pill').forEach(btn => btn.classList.remove('active')); e.target.classList.add('active'); washType = e.target.getAttribute('data-val');
        document.getElementById('ui-milk-wash').classList.toggle('hidden', washType !== 'milk'); document.getElementById('ui-fat-wash').classList.toggle('hidden', washType !== 'fat'); document.getElementById('wash-results').innerHTML = '';
    }));
    document.getElementById('btn-fat-type').addEventListener('click', () => { openSelectModal('FAT INTENSITY', [{label:'Standard (Butter/Coconut)',value:'standard'},{label:'Heavy (Bacon/Sesame)',value:'heavy'}], (v, l) => { fatIntensity = v; document.getElementById('btn-fat-type').innerText = l; }); });
    document.getElementById('calc-wash-btn').addEventListener('click', () => {
        triggerHaptic('heavy'); const res = document.getElementById('wash-results');
        if (washType === 'milk') res.innerHTML = `<h3 class="zone-header">CLARIFICATION (4:1)</h3><div class="result-row amber-glow"><span class="ing-name">Whole Milk Required</span><span class="ing-amount">${((parseFloat(document.getElementById('milk-punch-vol').value)||0)/4).toFixed(0)}ml</span></div><div class="result-row"><span class="ing-name text-muted text-sm">RULE: Pour Punch INTO the Milk.</span></div>`;
        else res.innerHTML = `<h3 class="zone-header">FAT EXTRACTION</h3><div class="result-row neon-cyan"><span class="ing-name">Warm Liquid Fat</span><span class="ing-amount">${((parseFloat(document.getElementById('fat-spirit-vol').value)||0) * (fatIntensity==='standard'?(240/700):(120/700))).toFixed(0)}g</span></div>`;
    });

    // --- LAB: ACID ADJUSTER & MODIFIERS ---
